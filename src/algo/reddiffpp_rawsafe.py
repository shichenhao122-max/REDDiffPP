...
class REDDiffPPRawSafe(Algo):
    """REDDiffPP variant with optional prior-weight clipping for mismatched raw MRI."""
    ...
    def __init__(
        ...
        super(REDDiffPPRawSafe, self).__init__(net, forward_op)
        self.net = net
    ...
    def inference(self, observation, num_samples=1, **kwargs):
        ...
        # Keep the original REDDiffPP zero-init behavior so comparisons stay clean.
        mu = torch.zeros(
            num_samples,
        ...
        for step in pbar:
            ...
            lam = self.lambda_fn(sigmas).to(device=device, dtype=mu.dtype)
            optimizer.zero_grad()
            ...
            gradient = gradient * self.observation_weight + prior_gradient
            mu.grad = gradient
